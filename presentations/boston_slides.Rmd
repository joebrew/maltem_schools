...
x <- performance %>%
  group_by(district, 
           ...
           year = paste0(year, '-', trimester)) %>% 
  summarise(val = length(which(value >= 10)) / length(value) * 100) %>%
  dplyr::select(district, year, val) %>%
  mutate(val = round(val, digits = 2)) %>%
  ungroup %>%
  mutate(district_grp = paste0(district, grp))
xt <- x %>% spread(key = year, value = val)
# tablify(xt)
```
...
ggplot(data = x %>% mutate(year = factor(year)),
       aes(x = year,
           y = val)) +
  geom_point(size = 7, aes(color = district)) +
  geom_line(alpha = 0.5,
            aes(group = district_grp,
                color = district)) +
  theme_cism() +
  scale_color_manual(name = '',
                     values = cols) +
...
## Event study visualization (performance)
...
fit <- fit %>% mutate(trimester = 2:6) %>%
  dplyr::select(trimester, estimate, lwr, upr)
...
## Event study visualization (absenteeism)
...
#   ungroup %>%
#   mutate(absenteeism_rate = absences / eligibles * 100)
fit <- lm(absence ~ + timetreat, data = model_data)
# Get confidence intervals
ci <- confint(fit)
ci <- data.frame(ci)
names(ci) <- c('lwr', 'upr')
